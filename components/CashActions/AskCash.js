...
import AskCashRecap from "../CashRecap/AskCashRecap";
...
export default function Transfer() {
  const { setOpenAskCash, openAskCashRecap, setOpenAskCashRecap, locale } =
  ...
  // If the receiver input contains a value, we fetch the users that match in the database
  useEffect(() => {
  ...
  const onSubmit = () => {
    if (amount && receiverName && motive) {
      setOpenAskCashRecap(true);
    } else {
      setWarning(true);
    }
  };

  // If a receiver is selected, we get his name, avatar and wallet key
  ...
  return !openAskCashRecap ? (
      ...
      <View style={styles.container}>
        <View style={styles.askCashHeader}>
          <View>
            <Pressable
              onPress={() => {
                setOpenAskCash(false), setReceiverFocus(null);
              }}
            >
              <Image
    ...
    <AskCashRecap
      ...
      notifSenderWalletId={senderWalletId}
      motive={motive}
